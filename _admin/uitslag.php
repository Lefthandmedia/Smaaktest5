...
//----------------------------------
$woningtype = array();
$woningtypeSQL = "SELECT * FROM user_woningtype";

$woningtypeResult = mysql_query($woningtypeSQL);

while($woningtypeRows = mysql_fetch_array($woningtypeResult))
{
    $woningtype[$woningtypenRows['id']] = $woningtypeRows['woningtype'];
}
//----------------------------------

$rn = 1;
if(isset($_POST['tag'] )&& $_POST['tag'] !="") {
//print "test - ".$_POST['tag'];
$locatieSQL = "SELECT
                app_tags.tag,
...
//
    ...
    $locatieSQL = "SELECT
                    ...
                    app_locaties.locatie AS locatie,
                    app_locaties.thumb AS thumb 
                    FROM app_locaties WHERE actief = 1";
}
...
$tagname = $row['tag'];
	...
	//print "</pre>";
		...
		if($_POST['woningtype'] && $_POST['woningtype'] != "")
        {
             $joinSQL .= " INNER JOIN user_woningtype     ON users.user_woningtype = user_woningtype.id ";
			 $filterSQL .= " AND users.user_woningtype = '".$_POST['woningtype']."' ";
            $filterString .= "woningtype is " . $opleidingen[$_POST['woningtype']] .", <br>";
        }
		
if($_POST['tag'] && $_POST['tag'] != "")
        {
          $filterString .= "Tag is " . $tagname.",<br>";
		}
...
<table width="100%" border="0">
  ...
  <tr>
    <td>Woningtype:</td>
    <td>&nbsp;<?php $db->pulldown_query("SELECT id, woningtype FROM user_woningtype",'woningtype'); ?></td>
  </tr>
  <tr>
    <td>Auto:</td>
