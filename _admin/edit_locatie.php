...
$sql = "SELECT * FROM app_locaties WHERE id = '".$_REQUEST['id']."'";
	...
	$r = mysql_query($sql);
		...
		while ($row = mysql_fetch_assoc($r)) {
			if ($row['actief'] == '1') { $actief = 'Actief'; }
			else $actief ='Inactief';
			
		if(!$row['thumb']) { $row['thumb'] = "dummy.jpg"; }
 		echo "<tr>\n";
		if ($row['actief'] == '1') {
...
if($succes == 1){ header("Location: edit_locatie.php?action=edit&id=".$_REQUEST['id']."");};
if($succes == 2) { header("Location: edit_locatie.php?action=edit&id=".$_REQUEST['locID'].""); }
