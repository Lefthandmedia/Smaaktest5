...
function eindscore_totaal(){
    ...
    for ($i = 0; $i < 3; $i++) {
        ...
        $totaalxml .= "<item id='" . $rn++ . "'><naam><![CDATA[" . $locaties[$i]['locatie_naam'] . "]]></naam><cijfer><![CDATA[" . str_replace(".", ",", round($locaties[$i]['gemiddelde'], 1)) . "]]></cijfer><image src='" . $locaties[$i]['thumb'] . "' /></item>";
...
function eindscore_user_totaal(){
    ...
    while ($row = mysql_fetch_assoc($r)) {
        ...
        $sql2 = "SELECT AVG($loc) AS gemiddeld FROM app_stemmentotaal WHERE user_id = '" . $_SESSION['user_id'] . "'";
        $r2 = mysql_query($sql2);
        //echo $sql2;
